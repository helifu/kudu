...
WARNING: The following upgrade process is only relevant when building from source code.

. Prepare the software.
  - Copy the `kudu-tserver`, `kudu-master` and `kudu` binaries from your build directory.
. Upgrade the tablet servers.
  - Set the unavailable time for every tablet server to a large value (2 hours or more) by gflag
...
----
  - Restart a tablet server and wait until it is online, then reset the gflag above to be 7200 again.
+
[NOTE]
====
Make sure the restarted tablet server is already online before resetting the gflag.
If the unavailable time is not extended, restarted tablet servers could be evicted from the cluster.
====
  - Repeat the previous step for the remaining tablet servers.
  - Restore the default gflag value (5 minutes) for every tablet server.
+
[source,bash]
----
