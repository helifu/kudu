...
. Upgrade the tablet servers.
  - Set the unavailable time for every tablet server to a large value (2x your expected
...
[NOTE]
====
If the unavailable time is not extended, Replicas hosted on restarted tablet servers could
