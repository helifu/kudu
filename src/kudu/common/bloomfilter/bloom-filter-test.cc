...
class CpuTempDisable {
public:
  CpuTempDisable(CPU::IntelMicroArchitecture flag)
    : flag_(flag), reenable_(false) {
    switch (flag)
    {
    case CPU::AVX:
      reenable_ = HashUtil::cpu_.has_avx();
      break;
    case CPU::AVX2:
      reenable_ = HashUtil::cpu_.has_avx2();
      break;
    default:
      return;
    }
    HashUtil::cpu_.EnableFeature(flag_, false);
  }
  ~CpuTempDisable() {
    if (reenable_) {
      HashUtil::cpu_.EnableFeature(flag_, true);
    }
  }
private:
  CPU::IntelMicroArchitecture flag_;
  bool reenable_;
};
...
void BfUnion(const BloomFilter* x, BloomFilter* y, bool* success) {
  ...
  BloomFilter::Or(x, y);
  {
    CpuTempDisable t1(CPU::AVX);
    CpuTempDisable t2(CPU::AVX2);
    Arena arena(1024, 1024*1024);
    uint8_t* directory = static_cast<uint8_t*>(arena.AllocateBytesAligned(pb_y.directory().size(), 16));
    memcpy(directory, pb_y.directory().data(), pb_y.directory().size());
    BloomFilter z(pb_y.log_heap_space(), directory);
    BloomFilter::Or(x, &z);
    *success = ((*y) == (z));
  }
  return;
...
// Computes intersection of 'x' and 'y'. Computes twice with AVX enabled and disabled and
// verifies both produce the same result. 'success' is set to true if both intersection
// computations returned the same result and set to false otherwise.
void BfIntersection(const BloomFilter* x, BloomFilter* y, bool* success) {
  BloomFilterPB pb_y;
  BloomFilter::ToPB(y, &pb_y);
  BloomFilter::And(x, y);
  {
...
TEST(TestImpalaBloomFilter, BloomFilterAnd) {
  BloomFilter bf1(BloomFilter::MinLogSpace(100, 0.01));
  BloomFilter bf2(BloomFilter::MinLogSpace(100, 0.01));

  for (int i = 0; i < 30; ++i) BfInsert(bf1, i);
  for (int i = 20; i < 50; ++i) BfInsert(bf2, i);

  bool success;
  BfIntersection(&bf1, &bf2, &success);
  ASSERT_TRUE(success) << "SIMD BloomFilter::Intersection error";
  for (int i = 0; i < 20; ++i) ASSERT_FALSE(BfFind(bf2, i)) << i;
  for (int i = 20; i < 30; ++i) ASSERT_TRUE(BfFind(bf2, i)) << i;
  for (int i = 30; i < 50; ++i) ASSERT_FALSE(BfFind(bf2, i)) << i;

  // Insert another value to aggregated BloomFilter.
  for (int i = 0; i < 10; ++i) BfInsert(bf2, i);

  // Apply back to BloomFilter and verify if aggregation was correct.
  BfIntersection(&bf1, &bf2, &success);
  ASSERT_TRUE(success) << "SIMD BloomFilter::Intersection error";
  for (int i = 0; i < 10; ++i) ASSERT_TRUE(BfFind(bf2, i)) << i;
  for (int i = 11; i < 20; ++i) ASSERT_FALSE(BfFind(bf2, i)) << i;
  for (int i = 20; i < 30; ++i) ASSERT_TRUE(BfFind(bf2, i)) << i;
  for (int i = 30; i < 50; ++i) ASSERT_FALSE(BfFind(bf2, i)) << i;
  ASSERT_FALSE(BfFind(bf2, 81));
}
